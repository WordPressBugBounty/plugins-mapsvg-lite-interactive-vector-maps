/**
 * Critical preloader CSS shown before mapsvg-bundle.css (and inside Shadow DOM).
 * Keep in sync with php/Front/MapPreloader.php.
 */

export const LOADING_STYLE_ATTR = "data-mapsvg-critical"
export const LOADING_STYLE_ATTR_VALUE = "loading"

export const LOADING_CRITICAL_CSS =
  ".mapsvg{position:relative;overflow:hidden}" +
...
export function ensureLoadingCriticalCss(
  target?: StyleHost | null,
  createElement?: (tag: string) => HTMLElement,
): void {
  const host = target ?? (typeof document !== "undefined" ? document.head : undefined)
  if (!host) {
    return
  }
  const selector = `style[${LOADING_STYLE_ATTR}="${LOADING_STYLE_ATTR_VALUE}"]`
  ...
  const factory =
    createElement ??
    (typeof document !== "undefined" ? (tag: string) => document.createElement(tag) : undefined)
  if (!factory) {
    return
  }

  const style = factory("style")
  style.setAttribute(LOADING_STYLE_ATTR, LOADING_STYLE_ATTR_VALUE)
  style.textContent = LOADING_CRITICAL_CSS
