...
import { geocode, getNestedValue, parseBoolean } from "@/Core/Utils"
...
export class LocationFormElement extends FormElement {
    ...
    if (_this.formBuilder.mapsvg.isGeo()) {
      ...
      thContainer.on("typeahead:select", (ev, item) => {
        ...
        const locationData = {
          ...
          img: this.getMarkerImage(formData),
          imagePath: this.getMarkerImage(formData),
        }
  ...
  setEditorEventHandlers() {
    ...
    $(this.domElements.edit).on("change", 'select[name="markerField"]', async (e) => {
      const fieldName = $(e.target).val()
      this.resetMarkersByField()
      const newOptions = await this.fillMarkersByFieldOptions(fieldName)
      this.setMarkersByField(newOptions)
    })
  ...
  async fillMarkersByFieldOptions(fieldName) {
    if (!fieldName) {
      return {}
    }
    if (fieldName.indexOf("Object.") === 0) {
      fieldName = fieldName.replace("Object.", "")
    }

    const options = await this.formBuilder.mapsvg.objectsRepository.getOptionsByField(fieldName)

    const markerOptions = {}
    if (options) {
      const markerImg = this.formBuilder.mapsvg.options.defaultMarkerImage
      const rows = []
      options.forEach((option) => {
        const img =
          this.markersByField && this.markersByField[option.value]
            ? this.markersByField[option.value]
            : markerImg
        rows.push(
          `<tr data-option-id="${option.value}"><td>${option.label}</td><td><button class="btn dropdown-toggle btn-outline-secondary mapsvg-marker-image-btn-trigger mapsvg-marker-image-btn"><img src="${img}" class="new-marker-img" /><span class="caret"></span></button></td></tr>`,
        )
        markerOptions[option.value] = img
      })
  ...
  renderMarkerSelector() {
    ...
    const images = markerImages.map((image) => {
      let buttonIsActive = false
      if (this.formBuilder.editMode) {
        buttonIsActive = image.default
      } else {
        buttonIsActive = currentImage == image.relativeUrl
      }
      return `<button class="btn btn-outline-secondary mapsvg-marker-image-btn mapsvg-marker-image-btn-choose ${buttonIsActive ? "active" : ""}"><img src="${image.relativeUrl}" /></button>`
    })
  ...
  toggleMarkerSelectorInLocationEditor(jQueryObj, e) {
    ...
    const images = markerImages.map(function (image) {
      return `<button class="btn btn-outline-secondary mapsvg-marker-image-btn mapsvg-marker-image-btn-choose ${currentImage == image.relativeUrl ? "active" : ""}"><img src="${image.relativeUrl}" /></button>`
    })
  ...
  getMarkerImage(data: { [key: string]: any }): string {
    ...
    if (this.markersByFieldEnabled) {
      const [objectType, ...field] = this.markerField.split(".")
      const dbField = field.join(".")
      fieldValue = getNestedValue(data, this.markerField)
      if (!fieldValue) {
